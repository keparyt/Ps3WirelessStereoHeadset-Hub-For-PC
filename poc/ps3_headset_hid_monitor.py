#!/usr/bin/env python3
"""PS3 Wireless Stereo Headset passive information receiver.

ONE-WAY / READ-ONLY:
    PC <- HID input reports <- Sony receiver <- headset

This program never sends HID output reports, feature reports, control commands,
or battery polling commands. It only reads incoming information.

Windows uses a native CreateFileW/ReadFile backend by default because it is a
lower-level receive path than the hidapi polling wrapper. The hidapi backend
is still available for comparison with --backend hidapi.
"""
...
class Monitor:
    ...
    def descriptor_dump(self, index: int, info: dict[str, Any]) -> None:
        ...
        try:
            ...
            print(
                ...
                f"len={len(descriptor)} -> {target.name}"
            )
    ...
    def process_report(self, index: int, info: dict[str, Any], report: bytes) -> None:
        ...
        # The console is intentionally useful without requiring any input from
        # the operator. Every changed raw report is visible, while B0 packets
        # also receive a decoded human-readable line.
        if changed:
            print(
                f"[{datetime.now().strftime('%H:%M:%S.%f')[:-3]}] "
                f"C{index:02d} PAGE=0x{page:04X} LEN={len(report):03d}  {hex_bytes(report)}"
            )

        if decoded is not None and decoded != getattr(self, "_last_printed_status", None):
    ...
    def hidapi_worker(self, index: int, info: dict[str, Any]) -> None:
        ...
        try:
            ...
            print(
                f"  [OPEN/HIDAPI] C{index:02d} page=0x{int(info.get('usage_page') or 0):04X}"
            )
        ...
        for index, info in enumerate(self.devices):
            if use_native:
                ...
                try:
                    ...
                    print(
                        f"  [OPEN/NATIVE] C{index:02d} "
                        f"page=0x{int(info.get('usage_page') or 0):04X} "
    ...
    def stop(self) -> None:
        if self.stop_event.is_set():
            # Still write the summary once when called after a KeyboardInterrupt.
            pass
        self.stop_event.set()
        ...
        for index, stats in self.stats.items():
            collections[str(index)] = dict(stats)
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        ...
        default="auto",
        help="Receive backend. 'auto' uses native Windows ReadFile on Windows.",
    )
    parser.add_argument(
        "--all-hid",
        action="store_true",
...
def main() -> int:
    ...
    if backend == "auto":
        backend = "native" if native_windows_available() else "hidapi"
